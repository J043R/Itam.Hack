...
function UserApp() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      console.log('User authenticated, checking form. hasProfile:', hasProfile, 'formFilledLegacy:', formFilledLegacy, 'filled:', filled);
      setFormFilled(filled);
    }
  }, [isAuthenticated]);
  ...
  console.log('Should show form?', isAuthenticated && !profileExists);
  
  // Если пользователь авторизован, но анкета не существует, показываем только анкету
  // Форма обязательна для заполнения - пользователь не может её закрыть
  if (isAuthenticated && !profileExists) {
    console.log('✅ SHOWING FORM - User authenticated but profile does not exist');
    return (
      <Form 
        isOpen={true} 
        onClose={() => {}} 
  ...
  console.log('✅ Rendering main app (form is filled)');
  ...
  return (
    <AdminLayout>
      <Routes>
        ...
        <Route path="/admin/hackathons/:hackathonId/edit" element={<EditHackathon />} />
